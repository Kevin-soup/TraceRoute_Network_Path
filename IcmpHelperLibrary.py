...
# Citation for the following messages:                                                                                 #
# Date: 03/07/2026                                                                                                     #
# Type: Online Registry                                                                                                #
# Adapted from textbook:                                                                                               #
# IANA (2026). Internet Control Message Protocol (ICMP) Parameters [Online Registry].                                  #
# https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml

icmp_messages = {
    0: {0: "ECHO RESPONSE - Destination Reached"},
...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            print(
                f"EXP: ID={self.getPacketIdentifier()}, Seq={self.getPacketSequenceNumber()}, Data={self.getDataRaw()}")
            print(
                f"ACT: ID={icmpReplyPacket.getIcmpIdentifier()}, "
                f"Seq={icmpReplyPacket.getIcmpSequenceNumber()}, Data={icmpReplyPacket.getIcmpData()}")
        ...
        def sendEchoRequest(self):
            ...
            try:
                ...
                if timeLeft <= 0:
                    ...
                    # Fetch description from dictionary
                    description = icmp_messages.get(icmpType, {}).get(icmpCode, "Unknown Error")

                    if icmpType == 11:                          # TTL Exceeded
                        print("  TTL=%d    RTT=%.0f ms    Type=%d (%s)    Code=%d    %s" %
                                (self.getTtl(), (timeReceived - pingStartTime) * 1000,
                                 icmpType, description, icmpCode, addr[0]))
    ...
    class IcmpPacket_EchoReply:
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            # Calculate RTT
            bytes_needed = struct.calcsize("d")
            timeSent = struct.unpack("d", self.__recvPacket[28:28 + bytes_needed])[0]
            rtt = round((timeReceived - timeSent) * 1000)

            # Fetching type and code
            icmpType = self.getIcmpType()
            icmpCode = self.getIcmpCode()
            ...
            if self.isValidResponse():
                print(f" TTL={ttl} RTT={rtt} ms Type={icmpType} Code={icmpCode} ({description}) from {addr[0]}")
            else:
                # Report error details
                print(f" [!] Invalid Response from {addr[0]}")
                ...
                if icmpType == 0:
                    ...
                    if self.getIcmpIdentifier_isValid() == False:
                        print(f"Identifier Mismatch.")
                else:
                    print(f"ICMP Error: {description}")
    ...
    # Type: Source code                                                                                                #
    # Adapted from:                                                                                                    #
    # GeeksforGeeks (2024). Ping and Traceroute implementation in Python.                                              #
    # https://www.geeksforgeeks.org/ping-in-python/                                                                    #
    #                                                                                                                  #
    # ################################################################################################################ #
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(numPackets):
            icmpPacket = self.IcmpPacket()
            icmpPacket.setIcmpTarget(host)

            icmpPacket.setTtl(255)
            icmpPacket.buildPacket_echoRequest(os.getpid() & 0xffff, i)

            response = icmpPacket.sendEchoRequest()
            if response is not None:
                icmpType, rttValue = response

                # Statistics for Echo responses
                if icmpType == 0 and rttValue is not None:
                    rttList.append(rttValue)

        # Ping report
        print(f"--- Ping statistics for {host} ---")
        received = len(rttList)
        if received > 0:
            loss = ((numPackets - received) / numPackets) * 100
            print(
                f" Packets: Sent={numPackets}, Received={received}, Lost={numPackets - received} ({loss}% loss)")
            print(f" Min={min(rttList)}ms, Max={max(rttList)}ms, Avg={sum(rttList) / received}ms")
        else:
            print(f" Packets: Sent={numPackets}, Received=0, Lost={numPackets} (100% loss)")

    def __sendIcmpTraceRoute(self, host):
        print("sendIcmpTraceRoute Started...") if self.__DEBUG_IcmpHelperLibrary else 0
        maxHops = 40

        # Iterate TTL up to maxHops
        for ttl in range(1, maxHops + 1):
            icmpPacket = self.IcmpPacket()
            icmpPacket.setTtl(ttl)
            icmpPacket.setIcmpTarget(host)
            icmpPacket.buildPacket_echoRequest(os.getpid() & 0xffff, ttl)

            # Send packet and get RTT
            result = icmpPacket.sendEchoRequest()

            # Check if destination reached
            if result is not None:
                if icmpPacket.getTtl() == ttl:
                    print(f"Destination {host} reached.")
                    break
...
def main():
    ...
    ## TRACEROUTE TEST
    icmpHelperPing.traceRoute("www.yahoo.co.jp")
